...
| 자산(색·이미지) 추가 후 | 빌드하면 런스크립트가 자동 생성 (수동: `./Parfait/Parfait/ParfaitModules/scripts/gen-assets.sh`) → 생성된 `Sources/UIComponent/Sources/{Colors,Assets}+.swift` 커밋 (손편집 금지) |
...
- 공용 모듈(`Common`·`Core`·`UIComponent`·`Routing`)은 AI가 임의 수정 금지 — 여러 기능이 의존해 충돌 위험. 수정 필요 시 **반드시 사람의 컨펌을 받고** 진행.
